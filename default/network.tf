...
output "network" { 
  value = {for segment in local.segments : segment.name => {
    name         = segment.name
    region       = var.resident.region
    display_name = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}"
    dns_label    = "${var.resident.label}${index(local.vcn_list, segment.name) + 1}"
    compartment  = contains(flatten(local.domains[*].name), "network") ? "${var.resident.name}_network_compartment" : var.resident.name
    stage        = segment.stage
    cidr         = segment.cidr
    gateways = {
      drg = {
        name     = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_router"
      ...
      internet   = {
        name     = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_internet"
      }
      nat        = {
        name     = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_translation"
      }
      service    = {
        name     = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_service"
        scope    = var.resident.osn == "ALL" ? "osn" : "storage"
        all      = local.osn_cidrs.all
        storage  = local.osn_cidrs.storage
      }
    }
    route_tables = {for subnet in local.subnets : subnet.name => { 
      display_name = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_${subnet.name}_table"
      stage        = subnet.stage
      route_rules  = flatten([for destination in local.port_filter[subnet.firewall].egress: [for zone in destination.zones : {
        ...
        network_entity = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_${destination.gateway}"
      }]])
    }}
    security_lists = {for subnet in local.subnets : subnet.name => { 
      display_name = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_${subnet.name}_filter"
      stage        = subnet.stage
    ...
    security_groups = {for firewall in local.firewalls : firewall.name => { 
      display_name = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_${firewall.name}_filter"
    }}
    ...
    subnets = {for subnet in local.subnets : subnet.name => {
      ...
      dns_label     = "${var.resident.label}${index(local.vcn_list, segment.name) + 1}${substr(subnet.name, 0, 3)}"
      display_name  = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_${subnet.name}"
      route_table   = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_${subnet.name}_table"
      security_list = "${var.resident.name}_${index(local.vcn_list, segment.name) + 1}_${subnet.name}_filter"
      stage         = subnet.stage
  ...
  }if segment.stage <= var.resident.stage}
}